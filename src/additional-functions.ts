...
export function addListener() {
  ...
  for (let i = 0; blockResult.length > i; i++) {
    const blockResultIndex = blockResult[i];
    if (blockResultIndex != undefined) {

      let resultContainer = blockResultIndex.querySelector('.result-container')
      let name = blockResultIndex.querySelector('.name');
      let resultImg = blockResultIndex.querySelector('.result-img');
      let favorites = blockResultIndex.querySelector('.favorites');
      if (resultContainer != null && name != null && resultImg != null && favorites != null) {
        ...
        buttonForm
          .addEventListener('click', (event) => {
            const blockFavorite = blockResultIndex.querySelector('div.favorites');
